...
## What the table says (2024 – Aug 2026, five funders, $784M of AI-risk grants)
...
the reading below is ours and, unless it says otherwise, uses the default variant:
programme splits applied only where their confidence is medium or high (`-allsplits`
applies the low-confidence ones too). Shares are
of the $784M labelled as AI-risk work; $5M labelled `not_ai` is reported but not in the base.
...
| | |
|---|---|
| **Where the money is** | Field-building proper — talent, courses, hubs, community — **31%**; governance (`6.5`) **24%**; cross-cutting AI-safety work whose subdomain the grant text does not say (`cross`, derived from a `field` risk label with a non-talent method label) 10%; misalignment (`7.1`) 10%; evaluations (`7.2`) 9%; AI-system security 4%. These six rows hold 89%; interpretability is next at 3%. This applies programme splits only where the grantee's own published mix is measured or reasonably weighted (6 organisations, $44M; `snapshots/funding/programme-splits-2026-08-20.csv`). The largest grantees — MATS, FAR AI, GovAI, IAPS, RAND — list programmes without weights; applying their equal-weight (low-confidence) shares too would touch $293M, move $195M, and give field-building 23%, cross-cutting 6%, misalignment 17%: the large talent programmes are mostly technical safety, which a grant label cannot see. That range is drawn on the page, not in the bars. |
| **Where the experts are** | By mean probability of catastrophic harm under business as usual, the Delphi top three are dangerous capabilities (`7.2`, 21.5%), weapons & cyberattacks (`4.2`, 21.0%) and power centralization (`6.1`, 18.0%); together those three rows get 10% of the money, almost all of it `7.2`. By share of experts naming a domain a top-3 concern: fraud & scams (`4.3`, 27%), power centralization (24%), then disinformation, false information and dangerous capabilities (22% each). |
| **The mismatches** | `6.1` power centralization: rank 3 by expected severity, rank 2 by concern, **$5.4M / 0.7%** under every reading and no hidden method bucket behind it (most of it FLI's power-concentration RFP). `4.2` weapons & cyber: rank 2 by severity, **$2.3M / 0.3%**; it would reach $26M / 3% only through the low-confidence AI-bio shares of MATS and CBAI — and it is not an artefact of scope: Coefficient's biosecurity and GCR funds were read grant by grant, and the 87 AI-related grants there are field-building, not AI-enabled bio or cyber defence. `7.4` interpretability: rank **24 of 24** as a *risk* in the expert survey, **$26M, fifth among subdomains** by money. |
| **Risk vs method** | Every grant also carries a *method* label (MIT's 23 mitigation controls plus seven research/talent/advocacy labels of ours). On grant labels alone the cross-table resolves the `7.4` puzzle: its $34M is **$30M of interpretability research** — a method, funded because it serves `7.1`/`7.2`, not because anyone rates opacity as a top harm. Likewise `7.2` is $61M of evaluations (`3.1`), `6.5` is $149M of risk-management and regulation work (`1.2`), `7.1` is $57M of empirical alignment research. (Under the split the cells of split grants are products of an organisation's risk and method shares — modelled, not observed — so the cross-table is read on grant labels.) By method the whole portfolio is: talent & community 30%, risk management / regulation 21%, empirical safety research 16%, evaluations 10%, advocacy & communication 6%, forecasting 4%, interpretability 3%, theory 3%. |
...
| **How noisy the money side is** | A second, independent labelling of 120 control grants agrees with the main risk labels on the exact subdomain 91% of the time, on the MIT domain 93%; the method labels have no control sample yet. Labels for "General support of X" rest on what X does, not on the grant text. The $789M reviewed mixes $708.3M granted (Coefficient, EA Funds), $73.6M recommended (SFF, FLI), and $7.5M raised (Manifund), by award year, with no cross-funder flow deduplication. Two Coefficient records for MATS of $26.9M each, eleven days apart, may be one grant listed twice. |
...
the `field` label is split at read time into `field` (method is talent and community) and `cross` (any other method: AI-safety work with no single subdomain), so "field-building" and "unclear" are not one row; `scope-decisions-2026-08-20.csv`: the per-grant in/out decision for the grants outside
...
per row), applied only to general-support or `field`/`cross`-labelled grants and, by default, only
where the confidence is medium or high (`--splits-min-confidence low` applies all,
`--no-splits` none).
